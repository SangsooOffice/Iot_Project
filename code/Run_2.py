    ...
    def TThread(self):
        ...
        while True:
            if image is not None and image_ok==1:
                ...
                center_x = int(width / 2) # 이미지의 중심 x 좌표

                # 잘라내려는 가로 폭을 정의합니다. 이 값은 양쪽으로 확장됩니다.
                crop_width = 200 

                left_x = max(0, center_x - crop_width) # 왼쪽 경계를 계산합니다. 이미지 경계를 넘지 않도록 합니다.
                right_x = min(width, center_x + crop_width) # 오른쪽 경계를 계산합니다. 이미지 경계를 넘지 않도록 합니다.

                cropped_image = image[:, left_x:right_x, :]
                #cv2.imshow('oridinal', cropped_image)
                        ...
                        if self.steering_angle >= 85 and self.steering_angle <= 95:
                            ...
                            self.motor_go(90,50)
                                ...
                                if self.steering_angle<20:
                                    self.motor_left(100,100)
                                else:
                                    self.motor_left(self.speedSet,self.speedSet)
                        elif self.steering_angle > 95:
                                if self.steering_angle>160:
                                    self.motor_right(100,100)
                                else:
